...
const add_product = async(req,res)=>{
    try {
      const {pname,description,reg_price,sales_price,cat,small_qty,medium_qty,large_qty}=req.body
  ...
  const product_block=async(req,res)=>{
    try {
        const {id}=req.query;
        const product=await productModel.findByIdAndUpdate({_id:id},{$set:{is_active:false}})
  ...
  const product_unblock=async(req,res)=>{
    try {
        const {id}=req.query;
    const product=await productModel.findByIdAndUpdate({_id:id},{$set:{is_active:true}})
    res.status(200).json({data:"success"})    
    } catch (error) {
        console.error(error)
    }
  ...
  const load_editproduct =async(req,res)=>{
    try{
      const {id}=req.query
      const dataproduct=await productModel.findById({_id:id})
