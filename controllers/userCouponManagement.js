...
const applyCoupon = async (req, res) => {
  try {
    ...
    const iscouponCodeValid = await couponModel.findOne({
      $and: [
        { couponCode: couponCode },
        { status: true },
        { couponlimit: { $lte: parseTotal } },
      ],
    });
    if (iscouponCodeValid) {
      req.session.couponid = iscouponCodeValid._id;
      console.log("couponid", req.session.couponid);
      const iscouponUsed = await userModel.findOne({
        _id: userid,
        coupon: { $in: [iscouponCodeValid._id] },
      });
...
const removeCoupon = async (req, res) => {
  try {
    ...
    const addbackAmount = await couponModel.findById(
      { _id: couponid },
      { reductionRate: 1, _id: 0 }
    );
    const removeCoupon = await userModel.findByIdAndUpdate(
      { _id: userid },
      { $pop: { coupon: -1 } }
    );
